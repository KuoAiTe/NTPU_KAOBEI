...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _previewWidth = DeviceWidth;
        _previewHeight = DeviceHeight;
...
- (void)initSavePreviewImageButton
{
    ...
    _saveImageButton.frame = CGRectMake(_previewWidth - 70, _previewHeight - 80, 49, 49);
    _saveImageButton.backgroundColor=RGB(255,255,255);
    [_saveImageButton setImage:[UIImage imageNamed:@"download.png"] forState:UIControlStateNormal];
    [_saveImageButton setImage:[UIImage imageNamed:@"okay.png"] forState:UIControlStateSelected];
    [_saveImageButton addTarget:self action:@selector(savePreviewImage) forControlEvents:UIControlEventTouchUpInside];
    _saveImageButton.layer.borderColor = [RGB(204, 204, 204) CGColor];
     _saveImageButton.layer.borderWidth = 1;
